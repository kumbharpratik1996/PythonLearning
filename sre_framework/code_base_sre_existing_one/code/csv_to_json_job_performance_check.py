...
python_logs = f"{parent_path}\logs\python"

if os.path.exists(f"{python_logs}"):
    shutil.rmtree(f"{python_logs}")
    os.mkdir(python_logs)
else:
    os.mkdir(python_logs)


# log file for this python script
log_file = f"{parent_path}\logs\python\csv_to_json_job_performance_check.log"
